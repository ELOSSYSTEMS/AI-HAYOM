...
def write_sitemap(repo: Path) -> None:
    catalog = json.loads((repo / "edition" / "catalog.json").read_text(encoding="utf-8"))
    urls = "\n".join(f"  <url><loc>https://aihayom.com/{number}</loc></url>" for number in catalog.get("editions", []))
...
def publish_local_commit(edition: dict, image_records: list[dict], state: dict, repo: Path) -> dict:
    ...
    add_paths = [str(target.relative_to(repo)), str(catalog_path.relative_to(repo)), "sitemap.xml"] + [str(asset.relative_to(repo)) for asset in assets]
    staged = False
    try:
